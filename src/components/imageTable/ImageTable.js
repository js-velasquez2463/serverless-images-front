    ...
    <div className="container mt-5">
      <div className="card">
        <div className="card-body">
          ...
          <button className="btn btn-primary" onClick={() => navigate('/add-image')}>Add New Image</button>
          <table className="table">
            ...
            <tbody>
              {currentItems.map((item, index) => (
                <tr key={index}>
                  <td>
                    <img src={item.s3_url} alt={"Image_s3_url"} className="img-thumbnail" style={{ width: '50px', height: '50px' }} />
                  </td>
                  <td>{item.image_name}</td>
                  <td>{item.user_id}</td>
                  <td><a href={item.s3_url} target="_blank" rel="noopener noreferrer">View Image</a></td>
                  <td>{item.user_metadata ? "Processed" : "Not Processed"}</td>
                  <td>{item.encrypted_metadata ? "Processed" : "Not Processed"}</td>
                </tr>
              ))}
            </tbody>
          </table>
